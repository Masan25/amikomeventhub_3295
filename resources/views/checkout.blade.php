...
<body class="bg-indigo-50/30 text-slate-900">
        ...
        <div class="grid grid-cols-1 gap-8">
            <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                ...
                <div class="flex gap-6 items-start">
                    <img src="{{ $event->poster_path ? asset($event->poster_path) : asset('assets/concert.png') }}" alt="{{ $event->title }}" class="w-24 h-24 rounded-2xl object-cover" onerror="this.onerror=null;this.src='https://placehold.co/120x120?text=Event';">
                    <div>
                ...
                <div class="mt-8 pt-6 border-t space-y-3">
                    <div class="flex justify-between text-slate-500"><span>Harga Tiket</span><span>Rp {{ number_format($event->price, 0, ',', '.') }}</span></div>
                    <div class="flex justify-between text-slate-500"><span>Biaya Layanan</span><span>Rp {{ number_format($serviceFee, 0, ',', '.') }}</span></div>
                    <div class="flex justify-between text-2xl font-black mt-4 pt-4 border-t"><span>Total Bayar</span><span class="text-indigo-600">Rp {{ number_format($total, 0, ',', '.') }}</span></div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                ...
                <form id="checkoutForm" class="space-y-6">
                    <div>
                        ...
                        <input id="customer_name" type="text" placeholder="Masukkan nama sesuai identitas"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="customer_email" type="email" placeholder="contoh@gmail.com"
                                class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                                required>
                        ...
                        <div>
                            ...
                            <input id="customer_phone" type="tel" placeholder="08xxxxxxx"
                                class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                                required>
                        </div>
                    </div>

                    <button type="button" onclick="showMidtrans()" class="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-xl shadow-xl shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">Bayar Sekarang</button>
    ...
    <div id="midtrans-overlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-6">
        <div class="bg-white w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl animate-bounce-in">
            <div class="bg-slate-50 p-6 flex justify-between items-center border-b">
                ...
                <button onclick="hideMidtrans()" class="p-2 hover:bg-slate-200 rounded-full"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-8 text-center">
                ...
                <h2 class="text-3xl font-black text-indigo-700 my-2">Rp {{ number_format($total, 0, ',', '.') }}</h2>
                <p class="text-xs text-slate-400">Order ID #{{ $orderId }}</p>
                <div class="mt-8 space-y-4">
                    <button onclick="goToTicket()" type="button"
                    ...
                    <button class="w-full py-4 border-2 border-indigo-100 rounded-2xl flex justify-between items-center px-6 hover:border-indigo-600 transition group opacity-50 cursor-not-allowed"><span class="font-bold">Virtual Account (BNI, BRI)</span><span class="text-indigo-400">→</span></button>
                    <button class="w-full py-4 border-2 border-indigo-100 rounded-2xl flex justify-between items-center px-6 hover:border-indigo-600 transition group opacity-50 cursor-not-allowed"><span class="font-bold">Kartu Debit/Kredit</span><span class="text-indigo-400">→</span></button>
                </div>
    ...
    function goToTicket() {
        const name = encodeURIComponent(document.getElementById('customer_name').value);
        const email = encodeURIComponent(document.getElementById('customer_email').value);
        const phone = encodeURIComponent(document.getElementById('customer_phone').value);

        window.location.href = "{{ route('ticket', $event->id) }}" + "?name=" + name + "&email=" + email + "&phone=" + phone;
    }
</script>
