...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        ...
        <p class="text-slate-400 text-sm font-bold uppercase mb-1">Stok Tiket</p>
        <h3 class="text-2xl font-black">{{ $totalStock ?? 0 }}</h3>
    </div>
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            <thead class="bg-slate-50 text-slate-400 uppercase text-[10px] font-black tracking-widest">
                <tr><th class="px-8 py-4">Pembeli</th><th class="px-8 py-4">Event</th><th class="px-8 py-4">Status</th><th class="px-8 py-4">Total</th></tr>
            </thead>
            <tbody class="divide-y border-t">
                @forelse(($latestTransactions ?? collect()) as $transaction)
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-8 py-6"><p class="font-bold uppercase tracking-wide text-sm">{{ $transaction->customer_name }}</p><p class="text-xs text-slate-400">{{ $transaction->customer_email }}</p></td>
                        <td class="px-8 py-6 font-medium text-slate-600">{{ $transaction->event->title ?? '-' }}</td>
                        <td class="px-8 py-6"><span class="px-3 py-1 bg-green-100 text-green-700 rounded-lg text-xs font-bold uppercase">{{ $transaction->status }}</span></td>
                        <td class="px-8 py-6 font-black text-indigo-600">Rp {{ number_format($transaction->total_price, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="px-8 py-10 text-center text-slate-400">Belum ada transaksi.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
