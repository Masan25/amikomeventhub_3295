    ...
    <div class="max-w-md w-full">
        ...
        <div class="ticket-card bg-white text-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl relative">
            <div class="p-8 bg-indigo-50 border-b-4 border-dashed border-indigo-100 text-center relative">
                ...
                <p class="text-sm text-slate-500 mt-2">{{ $event->category->name ?? '-' }}</p>
            ...
            <div class="p-8 space-y-8">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="font-bold text-lg">{{ $buyerName }}</p>
                    </div>
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase mb-1">Tanggal & Waktu</p>
                        <p class="font-bold text-lg">{{ $event->date->format('d M, H:i') }}</p>
                    </div>
                    <div>
                ...
                <div class="bg-slate-100 p-6 rounded-3xl flex flex-col items-center">
                    ...
                    <p class="mt-4 font-mono font-bold text-slate-800">{{ $ticketCode }}</p>
                </div>
            </div>

            <div class="px-8 pb-8 no-print">
                <button onclick="window.print()" class="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition">Cetak / Simpan PDF</button>
