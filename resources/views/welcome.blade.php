    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex flex-col lg:flex-row justify-between lg:items-end gap-8 mb-12">
            ...
            <div id="kategori" class="flex flex-wrap gap-2">
                <a href="{{ route('home') }}#events"
                    class="p-3 border rounded-xl hover:bg-white hover:shadow-md transition {{ request('category') ? '' : 'bg-indigo-600 text-white border-indigo-600' }}">Semua
                    Kategori</a>
                @foreach ($categories as $cat)
                    <a href="{{ route('home', ['category' => $cat->slug]) }}#events"
                        class="p-3 border rounded-xl hover:bg-white hover:shadow-md transition {{ request('category') == $cat->slug ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-700' }}">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
        ...
        @else
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-12 text-center">
                ...
                <p class="text-slate-500 mb-6">Belum ada event aktif pada kategori ini atau tanggal event sudah lewat.
                </p>
                <a href="{{ route('home') }}"
                    class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition">Tampilkan
                    Semua Event</a>
            </div>
        @endif
    </section>
    ...
    <section id="cara-pesan" class="max-w-7xl mx-auto px-6 py-10">
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-slate-600">
                <div class="p-6 rounded-2xl bg-slate-50"><strong>1. Pilih Event</strong><br>Pilih event yang ingin kamu
                    ikuti.</div>
                <div class="p-6 rounded-2xl bg-slate-50"><strong>2. Isi Data</strong><br>Lengkapi nama, email, dan
                    nomor WhatsApp.</div>
                <div class="p-6 rounded-2xl bg-slate-50"><strong>3. Terima Tiket</strong><br>E-ticket langsung muncul
                    setelah pembayaran berhasil.</div>
            </div>
        </div>
    </section>
        ...
        <section id="partners" class="max-w-7xl mx-auto px-6 py-20">
            <div class="text-center mb-12">
                ...
                <p class="text-slate-500 font-medium max-w-2xl mx-auto">
                    Platform AmikomEventHub berada pada kategori layanan event digital, reservasi tiket, dan manajemen
                    kegiatan kampus.
                </p>
            </div>
            ...
            @if ($partners->count() > 0)
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    @foreach ($partners as $partner)
                        <div
                            class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 flex flex-col items-center text-center hover:shadow-xl transition">
                            <img src="{{ $partner->logo_url ?: 'https://placehold.co/160x100?text=Partner' }}"
                                alt="{{ $partner->name }}" class="w-24 h-16 object-contain mb-4"
                                onerror="this.onerror=null;this.src='https://placehold.co/160x100?text=Partner';">
