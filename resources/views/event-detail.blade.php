...
<body class="bg-slate-50 text-slate-900">
    ...
    <nav class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center bg-white border-b sticky top-8 z-40 rounded-b-2xl shadow-sm">
        <a href="{{ route('home') }}" class="flex items-center gap-2">
            <div class="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold text-lg">AH</div>
            <span class="text-xl font-bold tracking-tight">AmikomEventHub</span>
        </a>
        <div class="flex gap-4">
            <a href="{{ route('home') }}#events" class="px-4 py-2 text-slate-600 font-medium hover:text-indigo-600">Cari Event</a>
    ...
    <main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                <img src="{{ $event->poster_path ? asset($event->poster_path) : asset('assets/concert.png') }}" alt="{{ $event->title }}" class="w-full rounded-[2.5rem] shadow-2xl border-8 border-white" onerror="this.onerror=null;this.src='https://placehold.co/600x800?text=Event';">
                <div class="mt-8 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="font-bold text-slate-800">{{ $event->organizer ?? 'AmikomEventHub' }}</p>
        ...
        <div class="lg:col-span-2 space-y-12">
            ...
            <div class="prose prose-slate max-w-none">
                ...
                <p class="text-lg text-slate-600 leading-relaxed mt-4">Pastikan data diri Anda benar saat melakukan pemesanan. E-ticket akan langsung terbit setelah pembayaran simulasi berhasil.</p>
            </div>
            ...
            <div class="bg-indigo-600 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden">
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    ...
                    <div>
                        <a href="{{ route('checkout', $event->id) }}" class="inline-block px-10 py-5 bg-white text-indigo-600 rounded-2xl font-black text-xl hover:scale-105 transition-transform shadow-xl">Pesan Sekarang</a>
                    </div>
                </div>
            ...
            <div class="space-y-4">
                ...
                <ul class="space-y-3 text-slate-500">
                    <li class="flex items-start gap-2"><svg class="w-5 h-5 text-green-500 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>E-Ticket akan dikirimkan otomatis setelah pembayaran berhasil.</li>
                    <li class="flex items-start gap-2"><svg class="w-5 h-5 text-green-500 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>Tiket dapat discan di pintu masuk.</li>
                    <li class="flex items-start gap-2 text-rose-500"><svg class="w-5 h-5 text-rose-500 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>Tiket yang sudah dibeli tidak dapat direfund.</li>
                </ul>
            </div>
        </div>
    </main>
    ...
    <footer class="bg-slate-900 text-slate-400 py-12 px-6 mt-20">
        <div class="max-w-7xl mx-auto text-center">
            <p>&copy; 2026 AmikomEventHub. Prototype Demo.</p>
        </div>
    </footer>
